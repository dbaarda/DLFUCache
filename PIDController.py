...
class PIDController(object):
  ...
  * There is a low-pass "RC" filter on the derivative to avoid derivative
    noise spikes.
  * The derivative low-pass filter means this works for step inputs with
  ...
  Attributes:
    ...
    Ld: The derivative low-pass filter timeconstant.
  ...
  def __init__(self, Kp, Ki, Kd, Ld=0.0):
    self.Kp = Kp
    self.Ki = Ki
    self.Kd = Kd
    self.Ld = Ld
  ...
  @classmethod
  def StandardForm(cls, Kp, Ti, Td, Ld=None):
    ...
    will default to Td/8.

    Args:
      Kp: The PID proportional gain.
      Ti: The PID integral time.
      Td: The PID derivative time.
      Ld: The derivative low-pass filter RC time (default: Td/8)
    ...
    if Ld is None:
        Ld = Td / 8.0
    return cls(Kp, 1.0/Ti, Td, Ld)

  @classmethod
  def ZiglerNichols(cls, Ku, Tu, Ld=None):
    ...
    Returns:
      An initialized PIDController.
    """
    return cls.StandardForm(0.6*Ku, Tu/2.0, Tu/8.0, Ld)
  ...
  def __repr__(self):
    return "PIDController(Kp=%5.3f, Ki=%5.3f, Kd=%5.3f, Ld=%5.3f)" % (
        self.Kp, self.Ki, self.Kd, self.Ld)
